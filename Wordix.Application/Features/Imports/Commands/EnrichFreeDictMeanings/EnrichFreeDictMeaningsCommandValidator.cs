...
/// <summary>
/// FreeDict meaning enrichment command validation kuralları.
/// 
/// 24 MB civarı TEI XML dosyasıyla çalışacağız.
/// Yine de parser limitlerini kontrollü tutuyoruz.
/// </summary>
public sealed class EnrichFreeDictMeaningsCommandValidator
    : AbstractValidator<EnrichFreeDictMeaningsCommand>
{
    private const int MaxParserRows = 500000;
    private const int MaxMessages = 1000;

    public EnrichFreeDictMeaningsCommandValidator()
    {
        RuleFor(command => command.SourceStream)
            .NotNull()
            .WithMessage("FreeDict TEI XML dosyası zorunludur.");

        RuleFor(command => command.SourceStream)
            .Must(stream => stream!.CanRead)
            .When(command => command.SourceStream is not null)
            .WithMessage("FreeDict TEI XML dosyası okunabilir olmalıdır.");
        ...
        RuleFor(command => command.AllowedLearningItemSources)
            .NotNull()
            .WithMessage("AllowedLearningItemSources null olamaz.");
        ...
        RuleFor(command => command.AllowedLearningItemSources)
            ...
            .WithMessage("AllowedLearningItemSources sadece CefrJ, Octanove veya Manual içerebilir.");
    }
}